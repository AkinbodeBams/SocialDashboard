...
class SocialDashBoardTest(APITestCase):
    ...
    def test_for_missing_keyword(self):
        ...
        self.assertEqual(
            response.json()['Message'],  "You Need To Enter At least A Keyword")

    def test_for_added_keyword(self):
        response = self.client.get(
            path=self.url + "?keyword=testing&network=twitter")
    ...
    def test_get_social_media_filter_by_date_range(self):
        response = self.client.get(
            path=self.url + "?keyword=testing&from_date=2022-10-05&to_date=2022-11-05")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_get_social_media_filter_by_multiple(self):
        response = self.client.get(
            path=self.url + "?keyword=testing&from_date=2022-10-05&to_date=2022-11-05&language=ar")
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_catch_error_on_invalid_filter(self):
        # city is the invalid filter here
        response = self.client.get(
            path=self.url + "?keyword=testing&from_date=2022-10-05&to_date=2022-11-05&city=greece")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
