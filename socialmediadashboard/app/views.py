    ...
    def get(self, request, *args, **kwargs):
        ...
        if all_keywords.exists():
            ...
            if from_date and to_date:
                try:
                    filtering = filtering.filter(
                        date_posted__range=(from_date, to_date))
                except ValidationError:
                    return Response({"status": False, "message": 'Bad date format, try YYYY-MM-DD'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            for i in keywords:
                ...
                if i not in distinct_rows or ((datetime.now(timezone.utc)) - (SocialDashboardData.objects.filter(keyword=i).last().modified_at)) > timedelta(hours=24):
                    caller.append(i)
            if len(caller) > 0:
